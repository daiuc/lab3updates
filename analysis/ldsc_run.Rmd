...
# only used once to construct gene length file
plan(multisession, workers = 4)
future_globals_maxSize = 30*1024*1024^2 # this is effectively 30GB
options(future.globals.maxSize = future_globals_maxSize)

# make gene length
gtf = fread("../heritability/data/gencode.v31.primary_assembly.annotation.pandas.df.20200108.txt")
gene_ids = filter(gtf, str_detect(gene_id2, "ENSG[0-9]+$") & 
                    gene_type %in% c("protein_coding") & 
                    feature %in% c("CDS")) %>% pull(gene_id2) %>% unique

calcGeneCDSLength = function(gtf.df, gene.id) {
  filter(gtf.df, gene_id2 == gene.id & 
         gene_type %in% c("protein_coding") & 
         feature %in% c("CDS")) %>%
      mutate(length = end-start + 1) %>%
      select(gene_id2, gene_name, start, end, exon_number, length) %>%
      group_by(gene_id2, gene_name, exon_number) %>% 
      summarise(length = sum(length), .groups = "drop") %>% 
      group_by(gene_id2, gene_name) %>% summarise(length = sum(length), .groups = "drop")
}

gene.lengths = future_map_dfr(gene_ids, ~calcGeneCDSLength(gtf, .x))
fwrite(gene.lengths, "../heritability/data/gencode.v31.primary.gene.CDS_Lengths.txt")
```

```{r message=FALSE, warning=FALSE, include=FALSE}
# read in data

gene.lookup = fread("../heritability/data/geneID_geneSymbol_lookup.txt", sep = "\t", col.names = c("id","gene")) %>% rename("gene_id"="id", "gene_nm"="gene")
eds = readxl::read_xlsx("../heritability/data/wang_et_al_2020_AJHG_eds_pLI_RVIS.xlsx")
gene.lengths = fread("../heritability/data/gencode.v31.primary.gene.CDS_Lengths.txt") %>%
  rename("gene_id"="gene_id2")
gtex = fread("../heritability/output/GTEx_v8_sigif_eQTL_summ.txt")
hsq = fread("../heritability/output/hsq.txt", sep="\t") %>% rename("gene_id"="gene")
```

```{r include=FALSE}
# massaging data, bringing all data into 1 dataframe
hsq = inner_join(hsq, gene.lookup)
remove_gene = group_by(hsq, gene_id) %>% summarise(n=n()) %>% filter( n > 2) %>% pull(gene_id)
hsq = filter(hsq, ! gene_id %in% remove_gene) %>% 
        group_by(gene_id) %>% 
        summarise(h2 = mean(h2), m_tot = sum(m_tot), .groups = "drop") %>%
        inner_join(gene.lookup, by = "gene_id")
hsq = inner_join(hsq[,c("gene_id","gene_nm","h2", "m_tot")], eds[, c("GeneSymbol", "EDS","pLI","RVIS")], by = c("gene_id" = "GeneSymbol"))
hsq = mutate_at(hsq, .vars = c("pLI", "RVIS"), .funs = "as.numeric")
hsq = filter(hsq, h2 > 0)
hsq = inner_join(hsq, gtex[, c("gene_id", "max.abs.slope_z", "N_QTLs")], by = "gene_id")
hsq = inner_join(hsq, gene.lengths[,-c("gene_name")], by = "gene_id")
```
...
```{r}
ggplot(hsq) + geom_density(aes(m_tot, ..density..), fill="royalblue", alpha=.5) + labs(x = bquote(M[tot]))
```

#### $h^2$ distribution

```{r, warning=FALSE, message=FALSE, }
ggplot(hsq) + geom_density(aes(x = h2, y = ..density..), fill = "coral", alpha = .5) + labs(x = bquote(h^2))
```
...
```{r}
cor.test(~ m_tot + h2, data = hsq)
```

```{r}
g1 = ggplot(hsq) + geom_boxplot(aes(h2, m_tot, group = cut_width(h2, .01))) + 
    labs(x = bquote(M[tot]), y = bquote(h^2))
g2 = ggplot(hsq) + geom_boxplot(aes(log10(length), m_tot, group = cut_number(length, 10))) + labs(y = bquote(M[tot]), x = bquote(log[10](length)))
...
pLI (probability of being loss-of-function intolerant) reflects the tolerance of a gene to the loss of function mutations. *A higher value of pLI indicates a gene's function is more likely to be abrogated under a loss-of-function mutation*. It is frequently used to prioritize candidate genes when analyzing whole exome or whole genome data. Note that pLI specifically measures the impact of loss of function variants, such as missense, frame shift, early stop codon, etc.

**gene length**

I filtered protein coding genes and computed gene length from Gencode hg38 V31 GTF file.

**max.abs.slope**

I downloaded cis-eQTL significant SNP-gene pair dataset from GTEx V8 analysis. Data from 49 single tissue specific datasets are merged. For this metric as well as the `N_QTL` metric, all significant SNPs are subject to `MAF` filtering consistent with heritability estimate (`0.05 < MAF < 0.95`). Maximum effect size is summarised for each gene using the regression slope (`slope`) column in the `*.eqtl_signifpairs.txt.gz` data files. The `slope` values are first taken as absolute value before max summarising by gene.

**GTEx V8 N_QTL**

N_QTL refers to the number of significant cis-eQTLs per gene.

#### Distributions of other gene metrics

```{r}
metrics = c("EDS", "pLI", "RVIS", "length", "max.abs.slope_z", "N_QTLs") %>% set_names
```

```{r}
gg = map2(metrics, 1:6, function(v,i) {ggplot(hsq) + geom_density(aes_string(v, "..density.."), fill = i, alpha = .5)})
gg[[4]] = gg[[4]] + coord_cartesian(xlim = c(0,5e5)) + scale_x_continuous(labels = scales::comma_format(scale = .001, suffix = "Kb"))
```
...
#### Can we observe correlation between $h^2$ and the other metrics?

```{r include=FALSE}

gg.sc = map(metrics,function(v,i) {ggplot(hsq, aes_string("h2", v)) + geom_point(alpha = .3) + geom_smooth(method = "lm", se = F) +
    labs(x = bquote(h^2))})
gg.sc$length = gg.sc$length + coord_cartesian(ylim = c(0,5e5)) + 
    scale_y_continuous(labels = scales::comma_format(scale = .001, suffix = "Kb"))
gg.sc$N_QTLs = gg.sc$N_QTLs
```

<br>

```{r, warning=FALSE, message=FALSE, fig.height=8, fig.width=10}
gridExtra::grid.arrange(grobs = gg.sc, nrow=3)
```
...
```{r}
data.frame(p = unlist(corr.p), cor.spearman = unlist(corr.cor)) %>% knitr::kable() %>% kableExtra::kable_styling()
```
...
gg.bx5 = ggplot(hsq, aes(h2, max.abs.slope_z)) +
  geom_boxplot(aes(group = cut_width(h2, .01))) +
  geom_text(aes(x=.02, y= max(max.abs.slope_z), label=paste("cor:", scales::comma(corr.cor$max.abs.slope, accuracy = .01), 
                            "p:", scales::scientific(corr.p$max.abs.slope_z))),color = 'blue')
...
I performed correlation test between $h^2$ and all the other metrics. `EDS`, `gene length`, and `N_eQTLs` showed the strongest correlation. All are inversely correlated with $h^2$.

-   `EDS` was previously noted to negatively correlate with cis-eQTLs. One explanation is that mammalian genes have evolved a robustness to cope with regulatory genetic variation (Wang et al.). Thus genes with high EDS would be better explained by genomewide variants, rather than cis variants. This perhaps explains why higher EDS genes might have lower cis $h^2$.

-   gene length:

    -   possibly because longer genes do not necessarily contributes to more qualifying SNPs, as a result longer genes are penalized for heritability?
    -   should we restrict to length of CDS only?

-   N_QTLs: strange that it's negatively correlated.

    -   Perhaps due to many more SNPs/eQTLs in the GTEx dataset relative to SNPs used for $h^2$ (eQTLgen x EUR hapmap3)
    -   eQTLgen only has blood samples, but GTEx more diverse

-   `RVIS` is positively correlated. Since a higher positive `RVIS` score means that there are likely more functional variants than expected (possible positive selection). Also from previous plot, it suggests that higher cis heritability may originate from the collective contribution of larger amount of common variants. From this standpoint, it makes sense that the two metrics should move in the same direction.
