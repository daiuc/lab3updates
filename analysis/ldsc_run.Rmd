...
gg.sc = map2(metrics, 1:6, function(v,i) {ggplot(hsq, aes_string("h2", v)) + 
    geom_point(color = i, alpha = .3) + 
    geom_smooth(method = "lm", se = F) +
    labs(x = bquote(h^2))})
gg.sc$length = gg.sc$length + coord_cartesian(ylim = c(0,5e5)) + 
    scale_y_continuous(labels = scales::comma_format(scale = .001, suffix = "Kb")) + geom_smooth(color = "red", se = F)
